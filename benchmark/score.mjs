export function scoreSample(actual, expected) {
  ...
  if (expected.memories_count > 0) {
    if (doc.memories[0].text !== expected.first_text) {
      return { passed: false, reason: `first text mismatch: got "${doc.memories[0].text}", expected "${expected.first_text}"` };
    }
    if (JSON.stringify(doc.memories[0].metadata.tags) !== JSON.stringify(expected.first_metadata_tags)) {
      return {
        passed: false,
